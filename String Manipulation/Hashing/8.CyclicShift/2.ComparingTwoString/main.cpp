//given two string, compare them in O(logN)
#include<bits/stdc++.h>
using namespace std;
...
void build2(string s){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(i) pref2[i].second = (pref2[i].second + pref[i - 1].second) % mod2;
    }
}
...
int lcp(int i, int j, int x, int y){
    ...
    while(lo <= hi){
        ...
        if(getHash(i, i + mid - 1) == getHash2(x, x + mid - 1)){
            ans = mid;
            lo = mid + 1;
        } 
        else {
